...
const ActionModal: React.FC = () => {
 ...
 const peerUsername = modalData.peerId
  ? activeUsers.find((user: User) => modalData.peerId === user.socketId)!
     .username
  : '';
  ...
  <div className={`modal ${modalData.isActive && 'is-active'}`}>
   ...
   <div className="modal-card">
    <section className="modal-card-body">
     <p className="has-text-centered is-size-4 my-2">
      {modalData.modalContent}
     </p>
     <div className="is-flex is-flex-direction-row is-justify-content-space-around">
      <button onClick={handleAcceptandCloseModal} className="button is-primary">
       {modalData.confirmBtnText}
      </button>
      <button onClick={handleDeclineandCloseModal} className="button is-danger">
       {modalData.declineBtnText}
      </button>
     </div>
    </section>
   </div>
   <button
    onClick={handleDeclineandCloseModal}
