...
const VideoDisplay: React.FC = () => {
  ...
  */
 const handleICEGatheringStateChangeEvent = useCallback(() => {
  if (myPeerConnectionRef.current) {
   console.log(
    `ice gathering state changed to: ${myPeerConnectionRef.current.iceGatheringState}`
   );
  }
 }, []);
 ...
 const createPeerConnection = useCallback(async () => {
  ...
  myPeerConnectionRef.current.oniceconnectionstatechange =
   handleICEConnectionStateChangeEvent;
  myPeerConnectionRef.current.onicegatheringstatechange =
   handleICEGatheringStateChangeEvent;
  myPeerConnectionRef.current.onsignalingstatechange =
  ...
  */
 const handleVideoChatOffer = useCallback(
  async (sdp: RTCSessionDescription) => {
    ...
    try {
     ...
     if (!webcamStreamRef.current) {
      ...
      try {
       ...
       console.error(
        'error in handleVideoChatOffer - local webcam stream',
        err
       );
      }
      ...
      if (webcamStreamRef.current) {
       try {
        ...
        console.error(
         'error in handleVideoChatOffer - webcam stream add tracks',
         err
        );
       }
      }
     ...
     try {
      ...
      await myPeerConnectionRef.current.setLocalDescription(
  ...
  * setRemoteDescription: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setRemoteDescription
  */
 const handleVideoChatAnswer = useCallback(
  async (sdp: RTCSessionDescription) => {
   if (myPeerConnectionRef.current) {
    try {
     const desc = new RTCSessionDescription(sdp);
 ...
 useEffect(() => {
  ...
  dispatch,
  startVideoChat,
  handleVideoChatOffer,
  handleVideoChatAnswer,
 ...
 return (
  <div className="is-flex is-flex-direction-row is-justify-content-space-evenly is-align-items-center">
   <div className="videoWrapper local m-1">
    <video
     ref={(el) => {
      localStreamRef.current = el;
     }}
     id="videoStream"
     autoPlay
    >
     There is a problem playing the video.
    </video>
   </div>
