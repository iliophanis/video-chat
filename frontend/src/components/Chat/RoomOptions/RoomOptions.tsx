...
const RoomOptions: React.FC = () => {
 ...
 const isTextChatVisible = useAppSelector(
  (state) => state.room.isTextChatVisible
 );
 ...
 return (
  <div className="mt-4 mb-4 is-flex is-flex-direction-row is-justify-content-center">
   <button className="button is-danger mr-1" onClick={handleEndClick}>
    End Chat
   </button>
   <button className="button is-primary mr-1" onClick={handleToggleChat}>
    {isTextChatVisible ? 'Hide Chat' : 'Show Chat'}
   </button>
  </div>
 );
};
